...
const Users = () => {
  ...
  const loadOrganizations = () => {
    ...
    if (storedUniversities) {
      ...
      const orgNames = universities.map((u: any) => u.name);
      setOrganizations(orgNames);
    }
  };

  const saveUsers = (updatedUsers: User[]) => {
    localStorage.setItem('masterAdminUsers', JSON.stringify(updatedUsers));
  ...
  const filteredUsers = users.filter(user => {
    ...
    const matchesOrganization = organizationFilter === 'all' || (user as any).organization === organizationFilter;
    return matchesSearch && matchesType && matchesStatus && matchesOrganization;
  });

  const exportUsers = () => {
    const csv = [
      ['Name', 'Email', 'Username', 'User Type', 'Status', 'Organization', 'Mobile No', 'Student/Staff ID', 'Date of Birth', 'Gender'].join(','),
      ...filteredUsers.map(u => [
        u.name,
        u.email,
        u.username,
            ...
            {filteredUsers.length === 0 ? (
              ...
              filteredUsers.map((user) => (
                <TableRow key={user.id}>
                  ...
                  <TableCell>
                    <Badge variant="outline">{(user as any).organization || '-'}</Badge>
                  </TableCell>
                  <TableCell>
